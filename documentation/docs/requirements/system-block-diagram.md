...
The ***Data Processing Layer*** contains multiple Google Gemini models leveraging the Gemini API in Slack, which is configured to generate channel day summaries and user summaries based on customized prompts. The models structure their results into a standardized summary output schema and inputs it into a MongoDB cluster containing multiple databases for each channel in storage.
