...
**NOTE:** The use of databases specifically for individual channels in a specific workspace allow for consistent storage/retrieval of raw data for messages and members, which contain all information provided by the Slack API directly. *Each channel database manages the same types of collections: 2 store unstructured data (messages, members) and 2 store structured data (day summaries, user summaries).* View class diagrams in System Overview for more details.